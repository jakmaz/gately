                    ...
                    {rows.map((row, i) => (
                        <tr key={i} className="border-b border-border/30 last:border-0 hover:bg-muted/10 transition-colors">
                            {row.map((cell, j) => {
                                ...
                                const isBinary = cell === 0 || cell === 1 || cell === '0' || cell === '1';

                                return (
                                    <td key={j} className={`px-6 py-3 ${isOutput ? 'font-bold' : ''}`}>
                                        {isBinary ? (
                                            <span className={`inline-flex items-center justify-center w-7 h-7 rounded-md font-mono text-xs shadow-inner shadow-black/20 ${cell == 1
